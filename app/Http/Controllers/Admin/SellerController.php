...
class SellerController extends Controller
{
    ...
    public function shop_certificate_update(Request $request, $id)
    {
        try{
            // Add debug statements
        // dd($request->all()); // Check if all form fields are being sent.
        // dd($request->file('gst_certificate')); // Check the uploaded file instance.
        // dd($request->file('gst_certificate')->isValid()); // Check if the uploaded file is valid.
            $request->validate([
            ...
            if ($request->gst_certificate) {
                $uploadedFile = $request->file('gst_certificate');
                
                if ($uploadedFile->isValid()) {
                    $destinationPath = 'shop/certificates/';
                    $newFileName = PdfManager::upload($destinationPath, $uploadedFile);
    
                    if ($newFileName) {
                        $shop->gst_certificate = $newFileName;
                    } else {
                        throw new \Exception('Failed to upload GST certificate.');
                    }
                } else {
                    throw new \Exception('Invalid GST certificate file provided.');
                }
            } 
    ...
    public function shop_pickup_added_on_shiprocket_status_update($id){

        $ShopPickupAddress = ShopPickupAddress::find($id);

        if (!$ShopPickupAddress) {
            return response()->json(['success' => false]);
        }
    
        $ShopPickupAddress->is_added_on_shiprocket = !$ShopPickupAddress->is_added_on_shiprocket;
        $ShopPickupAddress->save();
        ...
        //  // Toggle the value of is_added_on_shiprocket field
        //  $shop->is_added_on_shiprocket = !$shop->is_added_on_shiprocket;
        ...
        //  return back();
         ...
         //return response()->json(['message' => 'Shop pickup added on Shiprocket status updated successfully'], 200);
    }
}
